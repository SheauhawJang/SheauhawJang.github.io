...
function processInput() {
    ...
    worker = new Worker("mahjong-worker.js?v=202609060151");
    let task = 0;
...
function restartInput(i) {
    ...
    reworkers[i] = new Worker("mahjong-worker.js?v=202609060151");
...
function tilesImage(tids, bonus) {
    ...
    let width = 5;
    let max_card = 35;
    if (window.matchMedia("(width <= 512px)").matches) ((width = 7), (max_card = 20));
    if (tids.length >= max_card) width = 100 / max_card;
    else if (tids.length >= 100 / width) width = 100 / tids.length;
    if (bonus === 1) width *= 0.5;
    if (bonus === 2 && !window.matchMedia("(width <= 512px)").matches) width *= 0.5;
...
function processGBScore() {
    ...
    gb_worker = new Worker("mahjong-worker.js?v=202609060151");
...
function processJPScore() {
    ...
    jp_worker = new Worker("mahjong-worker.js?v=202609060151");
...
function processSCScore() {
    ...
    sc_worker = new Worker("mahjong-worker.js?v=202609060151");
...
function addReferenceMark() {
    ...
    if (window.matchMedia("(width <= 512px)").matches) box.closest('details').open = true;
}
...
function processGBSetting(id) {
    ...
    switch (id) {
        ...
        case 3:
            ...
            break;
        case 4:
            positive = [5, 10, 1, 6, 7, 9, 11, 3, 2, 4, 29, 38, 49, 58, 59, 63, 64, 65, 66, 67, 71, 74, 15];
            negative = [19, 41, 42, 43, 8, 12, "31,1", "31,2", 33, 34, 35, 14, 16, 44, 17, 18, 28, 47, 48, 51, 53, 56, 57, 70, 68, 69, 72, 73];
            radio = [undefined, 0, 0, undefined, 26, undefined, 0, 0, "46,1", "50,1", "52,2", "54,1", 55, "60,1", "61,1", "62,1"];
            score = [0, 0, 88];
            break;
    }
...
function processSCSetting(id) {
    // prettier-ignore
    const rules = [
        [[12], [1, 2, 3, 5, 16], [undefined, 2, 0, 0, 0, 0, 0, 0], 3], // MIL
        [[1, 12], [2, 3, 5, 16], [undefined, 2, 2, 5, 5, 0, 0, 0], 5], // Mahjong Soul
        [[1], [2, 3, 5, 12], [undefined, 2, 2, 5, 5, 0, 0, 0]], // Mahjong Pricess
